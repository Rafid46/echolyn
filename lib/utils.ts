...
export const configureAssistant = (voice: string, style: string) => {
  const validVoice = ["male", "female"].includes(voice) ? voice : "male";
  const validStyle = ["casual", "formal"].includes(style) ? style : "casual";
  ...
  const vapiAssistant: CreateAssistantDTO = {
    name: "Companion",
    firstMessage:
      "Hi, let's start the session. Today we'll be talking about {{topic}}.",
    transcriber: {
      provider: "deepgram",
      model: "nova-3",
      language: "en",
